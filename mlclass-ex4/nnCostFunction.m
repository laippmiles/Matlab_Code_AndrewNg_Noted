...
yk = zeros(num_labels, NumOfData); 
for i=1:NumOfData,
  yk(y(i),i)=1;
end
%创建类似ELM那种便签矩阵
% follow the form
J = (1/NumOfData) * sum ( sum (  (-yk) .* log(h_theta)  -  (1-yk) .* log(1-h_theta) ));
...
Reg = lambda  * (sum( sum ( t1.^ 2 )) + sum( sum ( t2.^ 2 ))) / (2*NumOfData);
...
for t=1:NumOfData,
    ...
    %注意BP的sigmoidGradient很简单，直接a（j）*delta（l+1）即可
	% skipping sigma2(0) 
...
% Regularization (here you go)
	...
	Theta1_grad(:, 1) = Theta1_grad(:, 1) ./ NumOfData;

	Theta1_grad(:, 2:end) = Theta1_grad(:, 2:end) ./ NumOfData + ((lambda/NumOfData) * Theta1(:, 2:end));


	Theta2_grad(:, 1) = Theta2_grad(:, 1) ./ NumOfData;

	Theta2_grad(:, 2:end) = Theta2_grad(:, 2:end) ./ NumOfData + ((lambda/NumOfData) * Theta2(:, 2:end));
